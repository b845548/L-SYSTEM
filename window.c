...
static void initData(void) {
  ...
  _sphere =gl4dgGenCylinderf(10, 1) ;// gl4dgGenSpheref(10, 10);
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 257, 257, 0, GL_GREEN, GL_FLOAT, hm);
  free(hm);
...
/*
  comme promis en cours, une fonction qui ajoute un objets aux
  coordonées (x, 1, z)
*/
void unObjet(GLfloat x, GLfloat z) {
  gl4duPushMatrix(); {
  ...
  glCullFace(GL_BACK);
  glBindTexture(GL_TEXTURE_2D, _compassTexId);
  glUniform1f(glGetUniformLocation(_pId, "texRepeat"), 5.0);
